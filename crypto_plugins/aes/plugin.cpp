#include "../../src/plugin/plugin_interface.h"
#include <iostream>
#include <cstring>
#include <vector>
#include <cstdint>

// ============================================================
// AES-128 (полная реализация)
// ============================================================

// S-box
static const uint8_t S_BOX[256] = {
    
};

// Обратный S-box
static const uint8_t INV_S_BOX[256] = {
    
};

static const uint8_t RCON[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

// Вспомогательные функции
static inline uint8_t xtime(uint8_t x) {
    return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00);
}

static void sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = S_BOX[state[i]];
    }
}

static void inv_sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = INV_S_BOX[state[i]];
    }
}

static void shift_rows(uint8_t* state) {
    uint8_t tmp;
    // row 1: сдвиг на 1
    tmp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = tmp;
    // row 2: сдвиг на 2
    tmp = state[2];
    state[2] = state[10];
    state[10] = tmp;
    tmp = state[6];
    state[6] = state[14];
    state[14] = tmp;
    // row 3: сдвиг на 3
    tmp = state[3];
    state[3] = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = tmp;
}

static void inv_shift_rows(uint8_t* state) {
    uint8_t tmp;
    // row 1: обратный сдвиг на 1
    tmp = state[13];
    state[13] = state[9];
    state[9] = state[5];
    state[5] = state[1];
    state[1] = tmp;
    // row 2: обратный сдвиг на 2
    tmp = state[2];
    state[2] = state[10];
    state[10] = tmp;
    tmp = state[6];
    state[6] = state[14];
    state[14] = tmp;
    // row 3: обратный сдвиг на 3
    tmp = state[7];
    state[7] = state[11];
    state[11] = state[15];
    state[15] = state[3];
    state[3] = tmp;
}

static void mix_columns(uint8_t* state) {
    uint8_t a, b, c, d;
    for (int i = 0; i < 4; i++) {
        a = state[i*4];
        b = state[i*4+1];
        c = state[i*4+2];
        d = state[i*4+3];
        state[i*4] = xtime(a) ^ xtime(b) ^ b ^ c ^ d;
        state[i*4+1] = a ^ xtime(b) ^ xtime(c) ^ c ^ d;
        state[i*4+2] = a ^ b ^ xtime(c) ^ xtime(d) ^ d;
        state[i*4+3] = xtime(a) ^ a ^ b ^ c ^ xtime(d);
    }
}

static void inv_mix_columns(uint8_t* state) {
    uint8_t a, b, c, d;
    for (int i = 0; i < 4; i++) {
        a = state[i*4];
        b = state[i*4+1];
        c = state[i*4+2];
        d = state[i*4+3];
        state[i*4] = xtime(xtime(xtime(a ^ c))) ^ xtime(xtime(a ^ d)) ^ xtime(b ^ c) ^ a ^ b ^ d;
        state[i*4+1] = xtime(xtime(xtime(b ^ d))) ^ xtime(xtime(b ^ a)) ^ xtime(c ^ d) ^ b ^ c ^ a;
        state[i*4+2] = xtime(xtime(xtime(c ^ a))) ^ xtime(xtime(c ^ b)) ^ xtime(d ^ a) ^ c ^ d ^ b;
        state[i*4+3] = xtime(xtime(xtime(d ^ b))) ^ xtime(xtime(d ^ c)) ^ xtime(a ^ b) ^ d ^ a ^ c;
    }
}

static void add_round_key(uint8_t* state, const uint8_t* key) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= key[i];
    }
}

// Расписание ключей
static void key_expansion(const uint8_t* key, uint8_t* round_keys) {
    memcpy(round_keys, key, 16);
    
    for (int i = 1; i <= 10; i++) {
        uint8_t* prev = round_keys + (i-1) * 16;
        uint8_t* cur = round_keys + i * 16;
        
        // RotWord
        uint8_t temp[4] = {prev[3], prev[0], prev[1], prev[2]};
        
        // SubWord
        for (int j = 0; j < 4; j++) {
            temp[j] = S_BOX[temp[j]];
        }
        
        // Rcon
        temp[0] ^= RCON[i-1];
        
        // XOR с предыдущим блоком
        for (int j = 0; j < 4; j++) {
            cur[j] = prev[j] ^ temp[j];
        }
        
        for (int j = 4; j < 16; j++) {
            cur[j] = prev[j] ^ cur[j-4];
        }
    }
}

// Шифрование блока
static void aes_encrypt_block(uint8_t* block, const uint8_t* round_keys) {
    add_round_key(block, round_keys);
    
    for (int round = 1; round <= 9; round++) {
        sub_bytes(block);
        shift_rows(block);
        mix_columns(block);
        add_round_key(block, round_keys + round * 16);
    }
    
    sub_bytes(block);
    shift_rows(block);
    add_round_key(block, round_keys + 10 * 16);
}

// Расшифрование блока
static void aes_decrypt_block(uint8_t* block, const uint8_t* round_keys) {
    add_round_key(block, round_keys + 10 * 16);
    
    for (int round = 9; round >= 1; round--) {
        inv_shift_rows(block);
        inv_sub_bytes(block);
        add_round_key(block, round_keys + round * 16);
        inv_mix_columns(block);
    }
    
    inv_shift_rows(block);
    inv_sub_bytes(block);
    add_round_key(block, round_keys);
}

// ============================================================
// PKCS#7 паддинг
// ============================================================

static std::vector<uint8_t> add_padding(const uint8_t* data, size_t size, size_t block_size) {
    size_t padding_len = block_size - (size % block_size);
    if (padding_len == 0) padding_len = block_size;
    
    std::vector<uint8_t> result(data, data + size);
    result.resize(size + padding_len);
    for (size_t i = 0; i < padding_len; i++) {
        result[size + i] = static_cast<uint8_t>(padding_len);
    }
    return result;
}

static bool remove_padding(std::vector<uint8_t>& data) {
    if (data.empty()) return false;
    
    uint8_t padding_len = data.back();
    if (padding_len == 0 || padding_len > data.size() || padding_len > 16) {
        return false;
    }
    
    for (size_t i = data.size() - padding_len; i < data.size(); i++) {
        if (data[i] != padding_len) return false;
    }
    
    data.resize(data.size() - padding_len);
    return true;
}

// ============================================================
// Интерфейс плагина
// ============================================================

static AlgorithmInfo info = {
    "aes",
    16,  // AES-128
    16   // блок 128 бит
};

extern "C" const AlgorithmInfo* get_algorithm_info() {
    return &info;
}

extern "C" size_t get_output_size(size_t input_size, int operation_type) {
    if (operation_type == OP_ENCRYPT) {
        size_t padding_len = 16 - (input_size % 16);
        if (padding_len == 0) padding_len = 16;
        return input_size + padding_len;
    } else {
        return input_size;
    }
}

extern "C" int encrypt(ConstBuffer key, ConstBuffer input, MutBuffer* output) {
    if (key.size != 16) {
        std::cerr << "AES: Invalid key size (expected 16 bytes)" << std::endl;
        return -1;
    }
    
    if (output->size < input.size + 16) {
        std::cerr << "AES: Output buffer too small" << std::endl;
        return -1;
    }
    
    // Расписание ключей
    uint8_t round_keys[11 * 16];
    key_expansion(key.data, round_keys);
    
    // Добавляем паддинг
    std::vector<uint8_t> padded = add_padding(input.data, input.size, 16);
    
    // Шифруем блоки
    for (size_t i = 0; i < padded.size(); i += 16) {
        aes_encrypt_block(padded.data() + i, round_keys);
    }
    
    memcpy(output->data, padded.data(), padded.size());
    output->size = padded.size();
    
    return 0;
}

extern "C" int decrypt(ConstBuffer key, ConstBuffer input, MutBuffer* output) {
    if (key.size != 16) {
        std::cerr << "AES: Invalid key size (expected 16 bytes)" << std::endl;
        return -1;
    }
    
    if (input.size % 16 != 0) {
        std::cerr << "AES: Input size not multiple of block size" << std::endl;
        return -1;
    }
    
    if (output->size < input.size) {
        std::cerr << "AES: Output buffer too small" << std::endl;
        return -1;
    }
    
    // Расписание ключей
    uint8_t round_keys[11 * 16];
    key_expansion(key.data, round_keys);
    
    // Копируем входные данные
    memcpy(output->data, input.data, input.size);
    
    // Расшифровываем блоки
    for (size_t i = 0; i < input.size; i += 16) {
        aes_decrypt_block(output->data + i, round_keys);
    }
    
    // Удаляем паддинг
    std::vector<uint8_t> result(output->data, output->data + input.size);
    if (!remove_padding(result)) {
        std::cerr << "AES: Invalid padding" << std::endl;
        return -1;
    }
    
    memcpy(output->data, result.data(), result.size());
    output->size = result.size();
    
    return 0;
}